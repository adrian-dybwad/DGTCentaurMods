...
import universalchess.paths as paths_module
...
def test_missing_fen_file_returns_default_without_creating_file(tmp_path, monkeypatch):
    ...
    # Patch FEN_LOG at the source of truth (paths module).
    monkeypatch.setattr(paths_module, "FEN_LOG", str(fen_path))

    if fen_path.exists():
        fen_path.unlink()

    result = paths_module.get_current_fen()

    assert result == paths_module.DEFAULT_START_FEN
